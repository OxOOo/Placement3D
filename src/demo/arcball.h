...
/// 设置4x4矩阵为0矩阵
inline static void Matrix4fSetZero(Matrix4fT* NewObj)
{
    NewObj->s.M00 = NewObj->s.M01 = NewObj->s.M02 =
    NewObj->s.M10 = NewObj->s.M11 = NewObj->s.M12 =
...
/// 设置4x4矩阵为单位矩阵
inline static void Matrix4fSetIdentity(Matrix4fT* NewObj)
{
...
public:
    ...
    /// 放大
    void zoomIn(double ratio) { if (zoomRate < 10) zoomRate *= ratio; }

    /// 缩小
    void zoomOut(double ratio) { if (zoomRate > 0.1) zoomRate /= ratio; }
